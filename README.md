...
しかしながらこれらの方法は、マイクロチップを読み込むために専用の機器が必要な点や、迷子札だと取れてしまう可能性などの問題があげられます。<br><br>
そこで、私達はこれらの課題をより簡単な解決を目指した迷子のペットを探すプラットフォームとして<br>
...
- 「ポイント」制度を導入する
    - 迷子のペットに飼い主が懸賞金のようなポイントを設定できるようにし、捜索に対するゲーム性と捜索者のモチベーションを高めます。
    - ポイントを用いて、迷子の通知や広告を出せるようにします。
    - 協力者側のメリットを担保することで、協力者数を増やし迷子探しコミュニティを結成する必要があると考えております。
    - ポイントは現金での購入やギフト券への換金を可能とし、一定のマージンを頂くことで持続可能な開発を目指します。
- 「プライベートチャット」機能を追加する
